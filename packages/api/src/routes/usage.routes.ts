...
usageRoutes.get('/daily', async (req, res) => {
  try {
    ...
    const byDate = new Map<string, { date: string; requests: number; inputTokens: number; outputTokens: number; totalTokens: number }>();
    for (const r of raw) {
...
usageRoutes.get('/by-user', async (req, res) => {
  try {
    ...
    // Get user names
    const userIds = usage.map(u => u.userId).filter((id): id is string => id !== null);
