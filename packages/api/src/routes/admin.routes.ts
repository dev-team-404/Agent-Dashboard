...
adminRoutes.get('/stats/global/overview', async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const [services, prePerService, preGlobal, perServiceTodayRows, globalTodayResult] = await Promise.all([
      ...
      prisma.$queryRaw<Array<{ service_id: string; today_requests: bigint; today_active_users: bigint }>>`
        SELECT
      ...
      `,
      prisma.$queryRaw<Array<{ today_active: bigint }>>`
    ...
    const [
      perServiceTotalsRows,
      perServiceAvg30dRows,
      perServiceAvg30dExclRows,
      globalTotalsResult,
      globalAvgResult,
    ] = await Promise.all([
 ...
 */
adminRoutes.get('/stats/global/cumulative-users-by-service', async (req: AuthenticatedRequest, res) => {
  try {
 ...
 */
adminRoutes.get('/stats/global/cumulative-tokens-by-service', async (req: AuthenticatedRequest, res) => {
  try {
    const days = Math.min(365, Math.max(1, parseInt(req.query['days'] as string) || 30));
 ...
 */
adminRoutes.get('/stats/global/dau-by-service', async (req: AuthenticatedRequest, res) => {
  try {
    const days = Math.min(365, Math.max(1, parseInt(req.query['days'] as string) || 30));
 ...
 */
adminRoutes.get('/stats/global/dept-usage-by-service', async (req: AuthenticatedRequest, res) => {
  try {
    const days = Math.min(365, Math.max(1, parseInt(req.query['days'] as string) || 30));
 ...
 */
adminRoutes.get('/stats/global/service-daily-requests', async (req: AuthenticatedRequest, res) => {
  try {
    const days = Math.min(365, Math.max(1, parseInt(req.query['days'] as string) || 30));
 ...
 */
adminRoutes.get('/stats/global/mau-by-service', async (req: AuthenticatedRequest, res) => {
  try {
 ...
 */
adminRoutes.post('/stats/batch', async (req: AuthenticatedRequest, res) => {
  try {
    const requests: Array<{ key: string; path: string; params?: Record<string, string> }> = req.body?.requests || [];
    ...
    const response: Record<string, { data: unknown; error: string | null }> = {};
