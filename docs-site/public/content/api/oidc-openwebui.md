...
> **Samsung SSO 등록 안내**: Samsung SSO(ADFS)에 등록해야 하는 redirect_uri는 **Auth Server 콜백 URL 1개만**입니다:
> ```
> https://a2g.samsungds.net:9050/oidc/sso-callback
> ```
> Open WebUI의 callback URL은 Samsung SSO에 등록할 필요 **없습니다**. 우리 Auth Server가 자체적으로 관리합니다.
...
```env
# Open WebUI 자체 URL (Open WebUI가 redirect_uri 생성에 사용)
...
| 환경변수 | 설명 |
|---------|------|
| `WEBUI_URL` | **Open WebUI의 외부 접근 URL** — 이 값으로 redirect_uri가 결정됨 (`{WEBUI_URL}/oauth/oidc/callback`) |
...
services:
  open-webui:
    ...
    ports:
      - "3000:8080"
    environment:
      # Open WebUI 자체 URL (redirect_uri 생성에 사용)
      WEBUI_URL: "http://a2g.samsungds.net:3000"
      # OIDC 인증
      ENABLE_OAUTH_SIGNUP: "true"
      OAUTH_PROVIDER_NAME: "Agent Platform"
...
docker run -d \
  --name open-webui \
  -p 3000:8080 \
  -e WEBUI_URL="http://a2g.samsungds.net:3000" \
