...
import { authenticateToken, signToken, isSuperAdminByEnv, extractBusinessUnit, requireSuperAdmin } from '../middleware/auth.js';
...
authRoutes.get('/me', authenticateToken, async (req, res) => {
    try {
        ...
        res.json({
            ...
            adminRole: admin ? admin.role : (isHardcodedFallback ? 'SUPER_ADMIN' : null),
...
async function syncClientsToAuthServer(clients) {
    try {
 ...
 */
authRoutes.get('/oidc-clients', authenticateToken, requireSuperAdmin, async (_req, res) => {
    try {
        const setting = await prisma.systemSetting.findUnique({ where: { key: 'oidc_clients' } });
        const clients = setting?.value ? JSON.parse(setting.value) : {};
 ...
 */
authRoutes.post('/oidc-clients', authenticateToken, requireSuperAdmin, async (req, res) => {
    try {
 ...
 */
authRoutes.put('/oidc-clients/:clientId', authenticateToken, requireSuperAdmin, async (req, res) => {
    try {
        ...
        if (Array.isArray(redirectUris)) {
            clients[clientId].redirectUris = redirectUris.map((u) => u.trim()).filter(Boolean);
        }
        let newSecret = null;
        if (regenerateSecret) {
 ...
 */
authRoutes.delete('/oidc-clients/:clientId', authenticateToken, requireSuperAdmin, async (req, res) => {
    try {
        const { clientId } = req.params;
