    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-pastel-800 tracking-tight">{t('knoxVerifications.title')}</h1>
            <p className="text-sm text-pastel-500 mt-0.5">
              {t('knoxVerifications.description')}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2.5 flex-wrap">
          <div className="flex items-center gap-2.5 px-4 py-2 bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            <span className="text-sm font-semibold text-pastel-700">{t('knoxVerifications.totalCount', { count: stats.total.toLocaleString() })}</span>
          </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-100/80 p-6">
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder={t('knoxVerifications.searchPlaceholder')}
              value={search}
        ...
        {showFilters && (
          <div className="mt-5 pt-5 border-t border-gray-100/80 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 animate-slide-down">
            <div>
              <label className="block text-xs font-semibold text-pastel-500 uppercase tracking-wider mb-2">{t('knoxVerifications.result')}</label>
              <select
                value={successFilter}
              ...
              >
                ...
                <option value="true">{t('knoxVerifications.success')}</option>
                <option value="false">{t('knoxVerifications.failure')}</option>
              </select>
            </div>

            <div>
              <label className="block text-xs font-semibold text-pastel-500 uppercase tracking-wider mb-2">{t('knoxVerifications.method')}</label>
              <select
                value={methodFilter}
          ...
          <table className="w-full" style={{ minWidth: '1000px' }}>
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100/80">
                <th className="px-5 py-4 text-left text-xs font-semibold text-pastel-500 uppercase tracking-wider">{t('knoxVerifications.colTime')}</th>
                <th className="px-5 py-4 text-left text-xs font-semibold text-pastel-500 uppercase tracking-wider">{t('knoxVerifications.colUser')}</th>
                <th className="px-5 py-4 text-left text-xs font-semibold text-pastel-500 uppercase tracking-wider">{t('knoxVerifications.colClaimedDept')}</th>
                <th className="px-5 py-4 text-left text-xs font-semibold text-pastel-500 uppercase tracking-wider">{t('knoxVerifications.colKnoxDept')}</th>
                <th className="px-5 py-4 text-left text-xs font-semibold text-pastel-500 uppercase tracking-wider">{t('knoxVerifications.colMethod')}</th>
                <th className="px-5 py-4 text-left text-xs font-semibold text-pastel-500 uppercase tracking-wider">{t('knoxVerifications.colEndpoint')}</th>
                <th className="px-5 py-4 text-left text-xs font-semibold text-pastel-500 uppercase tracking-wider">{t('knoxVerifications.colResult')}</th>
                <th className="px-5 py-4 text-left text-xs font-semibold text-pastel-500 uppercase tracking-wider">{t('knoxVerifications.colDetail')}</th>
              </tr>
            </thead>
                    ...
                    <td className="px-5 py-4">
                      {record.errorMessage ? (
                        <div className="relative">
                          ...
                          >
                            {expandedRows.has(record.id) ? t('knoxVerifications.collapse') : record.errorMessage}
                          </button>
        ...
        {pagination.totalPages > 1 && (
          <div className="px-6 py-4 border-t border-gray-100/80 flex items-center justify-between bg-gray-50">
            <p className="text-sm text-pastel-500">
              {t('knoxVerifications.paginationInfo', {
